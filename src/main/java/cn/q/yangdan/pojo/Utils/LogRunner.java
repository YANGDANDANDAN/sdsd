...
import cn.q.yangdan.pojo.LogUtils;
import cn.q.yangdan.pojo.LogWritable;
...
public class LogRunner extends ToolRunner implements Tool {
    ...
    private void setInputOutputPath(Job job) {
        ...
        //4. 设置输入路径和输出路径
        try {
            FileInputFormat.setInputPaths(job, new Path(GlobalConstants.INPUT_ETL_PREFIX + dir + "/*"));
    ...
    private void setArgs(String[] args, Configuration configuration) {
        ...
        if (args.length == 2 && args[0].equals("-d")) {
            ...
            //3.3 转换格式
            running_date = LogUtils.timestamp2DateString(yesterday_date.getTime() + "");
        }
    ...
    static class LogMapper extends Mapper<LongWritable, Text, LogWritable, NullWritable> {
        ...
        protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
            ...
            //2. 直接清洗
            LogWritable log = LogUtils.getLog(line);
            //3. 输出
